...
/**
 * As 18 composições escritas à mão, exatamente como estavam antes da gramática.
 *
 * Este arquivo não é usado em produção. Ele existe como referência do teste de
 * paridade: render.test.ts renderiza os 18 cards pelos dois caminhos e prova que
 * a troca por dado não mudou nenhum elemento, classe, texto ou propriedade CSS.
 *
 * Some quando a ponte com o Figma tiver dado uma volta completa e o dado passar
...
const t = (c: RenderContext, mult = 1) => `font-size:${(c.fsTitle * mult).toFixed(1)}px`;
const p = (c: RenderContext, mult = 1) => `font-size:${(c.fsBody * mult).toFixed(1)}px`;

export const LEGACY_LAYOUTS: LegacyLayout[] = [
